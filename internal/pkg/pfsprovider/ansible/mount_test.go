...
func Test_Mount(t *testing.T) {
	...
	assert.Equal(t, "dd if=/dev/zero of=/dac/job1/job/swap/client1 bs=1024 count=2048 && sudo chmod 0600 /dac/job1/job/swap/client1", fake.cmdStrs[4])
	assert.Equal(t, "losetup /dev/loop42 /dac/job1/job/swap/client1", fake.cmdStrs[5])
	assert.Equal(t, "mkswap /dev/loop42", fake.cmdStrs[6])
