...
func (fwa *dacctlActions) CreatePersistentBuffer(c CliContext) error {
	...
	request := dacctl.BufferRequest{c.String("token"), c.String("caller"),
		c.String("capacity"), c.Int("user"),
		c.Int("groupid"), dacctl.AccessModeFromString(c.String("access")),
		dacctl.BufferTypeFromString(c.String("type")), true}
	if request.Group == 0 {
